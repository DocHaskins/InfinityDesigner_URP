...
namespace doppelganger
{
    public class AssetManager : EditorWindow
    {
        ...
        private static string newShaderName = "DoppelGanger/Cloth";
    ...
    "_b", "_tpp_dirt", "_monster_fpp", "_02", "_03", "_tpp_02", "_tpp_03", "_02_fpp", "_02_tpp", "_03_fpp", "_03_tpp", "_03_r2_fpp", "_03_r2_tpp", "_03_r3_fpp", "_03_r3_tpp", "_03_r4_fpp", "_03_r4_tpp", "_03_r5_fpp", "_03_r5_tpp"
        ...
        public static void ConvertMaterials()
        {
            ...
            foreach (string materialFile in materialFiles)
            {
                ...
                if (mat != null)
                {
                    // Check if the material uses an URP/Lit or URP/Hair shader
                    if (mat.shader.name == "Universal Render Pipeline/Lit" || mat.shader.name == "Standard")
                    {
        ...
        private void CreateAndBindMaterials(int limit = 100)
        {
            ...
            foreach (var entry in textureFiles)
            {
                ...
                Debug.Log($"Found shaderToUse as {shaderToUse} for texture {baseName}");
        ...
        {
            Debug.Log($"Determining shader for baseName: {baseName}");

            bool hasIdx = textures.Any(file => file.EndsWith("_idx.jpg"));
            bool hasDif = textures.Any(file => file.EndsWith("_dif.jpg"));
            bool hasGra = textures.Any(file => file.EndsWith("_gra.jpg"));
            bool hasGrd = textures.Any(file => file.EndsWith("_grd.jpg"));

            Debug.Log($"Texture checks - hasIdx: {hasIdx}, hasDif: {hasDif}, hasGra: {hasGra}, hasGrd: {hasGrd}");

            if (baseName.Contains("decal"))
            {
                Debug.Log("Base name contains 'decal'. Using Decal shader.");
                return Shader.Find("DoppelGanger/Decal");
            }
            if (ShouldUseCustomShader(baseName))
            {
                Debug.Log("Using custom shader for base name.");
                return Shader.Find("DoppelGanger/Skin");
            }
            else if (ShouldUseHairShader(baseName))
            {
                Debug.Log("Using hair shader for base name.");
                return Shader.Find("DoppelGanger/Hair");
            }
            else if (hasGra || hasIdx || hasGrd)
            {
                Debug.Log("Using clothing shader based on texture presence.");
                return Shader.Find("DoppelGanger/Cloth");
            }
            else
            {
                Debug.Log("Using default shader: DoppelGanger/Clothing.");
                return Shader.Find("DoppelGanger/Cloth");
        ...
        private void CreateTexturesIndex()
        {
            ...
            string[] textureFiles = Directory.GetFiles(texturesFolderPath, "*.jpg", SearchOption.AllDirectories);
        ...
        private void AssignCustomShaderTextures()
        {
            ...
            foreach (var materialEntry in materials)
            {
                ...
                if (material.shader.name == "DoppelGanger/Cloth" ||
                    material.shader.name == "DoppelGanger/Hair" ||
                    material.shader.name == "DoppelGanger/Decal" ||
                    material.shader.name == "DoppelGanger/Skin")
                {
        ...
        "DoppelGanger/Skin"
            ...
            foreach (var textureFile in textureFiles)
            {
                ...
                else if (textureFile.Contains(baseName + "_nrm"))
                {
                    ...
                    {
                        material.SetTexture("Normal Map", texture);
                    }
                }
                ...
                else if (textureFile.Contains(baseName + "_ems"))
                {
                    ...
                    else
                    {
                        ...
                        Color _EmissionColor = new Color(8f, 8f, 8f, 8f); // High intensity color for HDR
                        material.SetColor("_EmissiveColor", _EmissionColor);
                        material.SetInt("_UseEmissiveIntensity", 1);
                        float emissionIntensity = Mathf.Pow(2f, 3f); // Convert 3 EV100 to URP's internal unit
